...
import backend.core.services.verificator as verificator
...
def test_exists_registration_request(
    app, client, verificator, email, value, status_code
):
    ...
    with mock.patch.object(
        ...
        with app.app_context():
            ...
            verificator.add_registration_request(
                "test@example.com", "test", "password", "user"
            )

            response = client.post(
                routes.EXISTS_REGISTRATION_REQUEST_ROUTE, json={"email": email}
            )
            ...
            if value.get("exists") == True:
                ...
                assert (
                    verificator.registration_requests.get(user.id).get("code")
                    == "123456"
                )
                assert (
                    verificator.registration_requests.get(user.id).get("timestamp")
                    is not None
                )
                assert (
                    verificator.registration_requests.get(user.id).get("failures") == 0
                )
...
def test_exists_change_password_request(
    db, app, client, verificator, email, value, status_code
):
    def mock_generate_verification_code():
        return "123456"
    ...
    with mock.patch.object(
        ...
        with app.app_context():
            ...
            response = client.post(
                routes.EXISTS_CHANGE_PASSWORD_REQUEST_ROUTE, json={"email": email}
            )
            ...
            if value.get("exists") == True:
                ...
                assert (
                    verificator.change_password_requests.get(user.id).get("code")
                    == "123456"
                )
                assert (
                    verificator.change_password_requests.get(user.id).get("timestamp")
                    is not None
                )
                assert (
                    verificator.change_password_requests.get(user.id).get("failures")
                    == 0
                )
                assert (
                    verificator.change_password_requests.get(user.id).get(
                        "new_password"
                    )
                    == "password"
                )
...
def test_exists_delete_account_request(
    db, app, client, verificator, email, value, status_code
):
    def mock_generate_verification_code():
        return "123456"
    ...
    with mock.patch.object(
        ...
        with app.app_context():
            ...
            response = client.post(
                routes.EXISTS_DELETE_ACCOUNT_REQUEST_ROUTE, json={"email": email}
            )
            ...
            if value.get("exists") == True:
                ...
                assert (
                    verificator.delete_account_requests.get(user.id).get("code")
                    == "123456"
                )
                assert (
                    verificator.delete_account_requests.get(user.id).get("timestamp")
                    is not None
                )
                assert (
                    verificator.delete_account_requests.get(user.id).get("failures")
                    == 0
                )


@pytest.mark.parametrize(
    "email, code, value, status_code",
    [
        (
            "test@example.com",
            "123456",
            {"message": "Verified registration request"},
            200,
        ),
        ...
        (
            ...
            "123456",
            {"error": "No registration request found for this email"},
            404,
        ),
...
def test_verify_registration_request(
    app, client, verificator, email, code, value, status_code
):
    def mock_generate_verification_code():
        return "123456"
    ...
    with mock.patch.object(
        ...
        with app.app_context():
            ...
            response = client.post(
                routes.VERIFY_REGISTRATION_REQUEST_ROUTE,
...
@pytest.mark.parametrize(
    ...
    [
        (
            "test@example.com",
            "123456",
            {"message": "Verified change password request"},
            200,
        ),
        ("   ", "123456", {"error": "Invalid input data"}, 400),
        ("", "123456", {"error": "Invalid input data"}, 400),
        ...
        (
            "notfound@example.com",
            "123456",
            {"error": "No change password request found for this email"},
            404,
        ),
        ("test@example.com", "321341", {"error": "Invalid code"}, 400),
    ],
...
def test_verify_change_password_request(
    app, db, client, verificator, email, code, value, status_code
):
    def mock_generate_verification_code():
        return "123456"
    ...
    with mock.patch.object(
        ...
        with app.app_context():
            ...
            response = client.post(
                routes.VERIFY_CHANGE_PASSWORD_REQUEST_ROUTE,
                json={"email": email, "code": code},
            )
...
@pytest.mark.parametrize(
    ...
    [
        (
            "test@example.com",
            "123456",
            {"message": "Verified delete account request"},
            200,
        ),
        ("   ", "123456", {"error": "Invalid input data"}, 400),
        ("", "123456", {"error": "Invalid input data"}, 400),
        ...
        (
            "notfound@example.com",
            "123456",
            {"error": "No delete account request found for this email"},
            404,
        ),
        ("test@example.com", "321341", {"error": "Invalid code"}, 400),
    ],
...
def test_verify_delete_account_request(
    ...
    with mock.patch.object(
        ...
        with app.app_context():
            ...
            response = client.post(
                routes.VERIFY_DELETE_ACCOUNT_REQUEST_ROUTE,
                json={"email": email, "code": code},
            )
...
@pytest.mark.parametrize(
    "email, password, username, value, status_code",
    [
        (
            "test@example.com",
            "TollesPasswort123!",
        ...
        (
            "alreadyinverificator@example.com",
            "TollesPasswort123!",
            "test",
        ...
        (
            "test@example.com",
            "TollesPasswort123!",
            "teniuasdhjashdojashdkjahskdjhaskjdhakjshdkjahs",
            {"error": "Invalid username"},
            400,
        ),
        (
...
def test_user_registration(
    client, app, db, verificator, email, password, username, value, status_code
):
    with app.app_context():
        import core.utils.routes as routes
        ...
        if email == "alreadyinverificator@example.com":
            verificator.add_registration_request(
                "alreadyinverificator@example.com", "test", "password", "user"
            )
        ...
        response = client.post(
            routes.USER_REGISTRATION_ROUTE,
        ...
        if status_code == 200:
            ...
            assert (
                verificator.registration_requests.get(user.id).get("code") is not None
            )
            assert (
                verificator.registration_requests.get(user.id).get("timestamp")
                is not None
...
@pytest.mark.parametrize(
    "email, password, value, status_code",
    [
        (
            "test@example.com",
            "TollesPasswort123!",
...
def test_user_login(client, app, db, email, password, value, status_code):
    ...
    with mock.patch(
        "core.api.users.create_access_token", side_effect=mock_create_access_token
    ):
        with mock.patch(
            "core.models.users.check_password_hash", side_effect=mock_check_hashed_value
        ):
            with mock.patch(
                "core.api.users.decode_token", side_effect=mock_decode_token
            ):
                with mock.patch(
                    "core.api.users.create_refresh_token",
                    side_effect=mock_create_refresh_token,
                ):
                    with app.app_context():
                        import core.utils.routes as routes
                        ...
                        user = users.User(
                            username="test",
                            email="test@example.com",
                            password_hash="TollesPassword123!",
                            role="user",
                        )
                        db.session.add(user)
                        db.session.commit()
                        ...
                        if status_code == 200:
                            ...
                            assert (
                                mock_decode_token(response.json["access_token"])["sub"]
                                == "test@example.com"
                            )
                            assert response.json["refresh_token"] is not None
                            assert (
                                mock_decode_token(response.json["refresh_token"])["sub"]
                                == "test@example.com"
                            )
...
def test_token_refresh(client, refresh_token):
    ...
    response = client.post(
        "/user/token/refresh",
        headers={"Authorization": f"Bearer {preset_refresh_token}"},
    )
...
def test_token_email(client):
    ...
    response = client.get(
        "/user/token/email", headers={"Authorization": f"Bearer {preset_refresh_token}"}
    )
...
@pytest.mark.parametrize(
    "email, new_username, value, status_code",
    [
        (
            "test@example.com",
            "newusername",
...
def test_change_username(client, app, db, email, new_username, value, status_code):
    ...
    with app.app_context():
        ...
        response = client.post(
            routes.CHANGE_USERNAME_ROUTE,
            json={"email": email, "new_username": new_username},
        )
...
@pytest.mark.parametrize(
    "email, new_password, value, status_code",
    [
        (
            "test@example.com",
            "TollesPasswort123!",
        ...
        (
            "alreadyinverificator@example.com",
            "TollesPasswort123!",
            {"error": "Change password request already exists"},
            409,
        ),
...
def test_user_change_password(
    client, app, db, verificator, email, new_password, value, status_code
):
    with app.app_context():
        import core.utils.routes as routes
        ...
        user = users.User(
            username="test",
            email="alreadyinverificator@example.com",
            password_hash="password",
            role="user",
        )
        db.session.add(user)
        ...
        verificator.add_change_password_request(
            "alreadyinverificator@example.com", "password"
        )

        response = client.post(
            routes.CHANGE_USER_PASSWORD_ROUTE,
        ...
        if status_code == 200:
            ...
            assert (
                verificator.change_password_requests.get(user.id).get("new_password")
                is not None
            )
            assert verificator.exists_change_password_request(email) == True
            assert (
                encryption.check_password_hash(
                    new_password,
            ...
            assert (
                verificator.change_password_requests.get(user.id).get("failures") == 0
            )
            assert (
                verificator.change_password_requests.get(user.id).get("timestamp")
                is not None
...
def test_check_password(client, app, db, email, password, value, status_code):
    with app.app_context():
        ...
        response = client.post(
            routes.CHECK_PASSWORD_ROUTE, json={"email": email, "password": password}
        )
