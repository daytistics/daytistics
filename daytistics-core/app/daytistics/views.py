...
def edit_activity_entry(request, daytistic_id):
	...
	activity_entry = get_object_or_404(
		ActivityEntry, id=activity_entry_id, daytistic=Daytistic.objects.get(id=daytistic_id)
	)
	...
	if duration_timedelta + activity_entry.daytistic.get_total_duration(
		human_readable=False
	) > timedelta(hours=24):
		return HttpResponse(
			f"<p class='text-red-500'>{_('Error: The total duration of activities can\'t be more than 24 hours')}</p>",
			status=200,
		)
...
def toggle_important(request, daytistic_id):
	...
	if not Daytistic.objects.filter(user=request.user, id=daytistic_id).exists():
		return HttpResponse('Daytistic not found', status=200)
