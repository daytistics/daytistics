...
class TestDaytisticModel:
	...
	def test_important_field(self):
		user = CustomUserFactory()
		daytistic_important = DaytisticFactory(user=user, important=True)
		assert daytistic_important.important == True

		daytistic_not_important = DaytisticFactory(user=user, important=False)
		assert daytistic_not_important.important == False

	@freezegun.freeze_time('2024-08-24 06:38:18')
	def test_created_at(self):
		specific_date = timezone.now()
		user = CustomUserFactory()
		daytistic_with_fixed_created_at = DaytisticFactory(user=user, created_at=specific_date)
	...
	def test_updated_at(self):
		specific_date = timezone.now()
		user = CustomUserFactory()
		daytistic_with_fixed_updated_at = DaytisticFactory(user=user, updated_at=specific_date)
