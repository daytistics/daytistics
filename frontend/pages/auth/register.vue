  ...
  <div class="min-h-screen bg-gradient-to-br from-green-400 to-blue-500 flex items-center justify-center p-4">
    <div class="max-w-md w-full bg-white rounded-lg shadow-xl overflow-hidden">
      <div class="p-6 sm:p-8">
        ...
        <h2 class="text-2xl font-bold text-center text-gray-700 mb-6">Sign up to create account</h2>
          ...
          <div>
            ...
            <div class="mt-2">
              <input placeholder="Full Name" type="text"
                ...
                v-model="form.username.value" name="user_name" />
            </div>
          </div>
          <div>
          ...
          <div>
            ...
            <div class="mt-2">
              <input placeholder="Password" type="password"
                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                v-model="form.password1.value" name="password" />
            </div>
          </div>
          <div>
            <div class="flex items-center justify-between">
            ...
            <div class="mt-2">
              <input placeholder="Password" type="password"
                class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                v-model="form.password2.value" name="password" />
            </div>
          </div>
          <div>
            <button type="submit"
      ...
      <div class="px-6 py-4 bg-gray-50 border-t border-gray-200 sm:px-8">
        <p class="text-xs leading-5 text-gray-500">
          ...
          <a href="/auth/login" class="font-medium text-green-600 hover:text-green-500">Log In</a>
        </p>
      </div>
    </div>
  </div>
...
function useForm() {
  ...
  const validate = () => {
    if (!isValidUsername(username.value)) {
      showErrorModal({
        message: 'Please enter a valid username. It must be at least 4 characters long and contain only alphanumeric characters.',
      });
      return false;
    }
    ...
    if (!isValidPassword(password1.value)) {
      showErrorModal({
        message: 'Please enter a valid password. It must be at least 8 characters long and contain at least one uppercase letter, one lowercase letter, one number, and one special character.',
      });
      return false;
    }
